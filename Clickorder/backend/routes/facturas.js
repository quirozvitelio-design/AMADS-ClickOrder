...
router.post("/:pedido_grupo", async (req, res) => {
  ...
  try {
    ...
    if (!cfgResult.recordset.length) {
      return res.status(400).json({
        mensaje:
          "Configure primero los datos de empresa en Configuración"
      })
    }
    ...
    const correlativo =
      contResult.recordset[0].total + 1

    const numero_factura =
      `FAC-${String(correlativo).padStart(4, "0")}`

    const codigoGeneracion = generarCodigo()
    const numeroControl =
      generarNumeroControl(correlativo)
    ...
    const dte = {
      ...
      emisor: {
        ...
        nombreComercial:
          cfg.nombre_comercial || cfg.nombre,

        codActividad:
          cfg.cod_actividad || "47191",
        ...
        codEstableMH:
          cfg.cod_estable_mh || "M001",

        codPuntoVentaMH:
          cfg.cod_punto_venta_mh || "P001"
      },
      ...
      receptor: {
        ...
        direccion: {
          departamento:
            items[0].departamento || "12",

          municipio:
            items[0].municipio || "17",
...
router.get("/:id/pdf", async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${factura.numero_factura}.pdf"`
    )
    ...
    doc
      ...
      .fillColor("#666")
      .text(
        `${emisor.nombre} | ${factura.numero_factura}`,
        L,
        y,
        {
          width: W,
...
router.get("/:id/json", async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${factura.numero_factura}.json"`
    )
