    ...
    <div class="page-content">
      ...
      <div v-if="vistaActiva === 'resumen' && ordenConfirmada" class="resumen-orden">
        ...
        <div class="resumen-footer">
          ...
          <div v-if="ordenConfirmada.factura" class="rf-row factura-row">
            <span>🧾 Factura generada</span>
            <strong class="factura-numero">{{ ordenConfirmada.factura.numero_factura }}</strong>
          </div>
        </div>
        ...
        <!-- BOTONES DESCARGA FACTURA -->
        <div v-if="ordenConfirmada.factura" class="factura-download-row">
          <button class="btn-dl-pdf" @click="descargarFacturaPDF(ordenConfirmada.grupo)">
            ⬇ Descargar PDF
          </button>
          <button class="btn-dl-json" @click="descargarFacturaJSON(ordenConfirmada.grupo)">
            { } Descargar JSON
          </button>
        </div>
      </div>
      ...
      <div v-if="vistaActiva === 'pedidos'">
        ...
        <div v-else class="pedidos-lista">
          <div v-for="(grupo, key) in pedidosAgrupados" :key="key" class="grupo-card">
              ...
              <div class="grupo-id-wrap">
                ...
                <div>
                  ...
                  <span class="grupo-codigo">
                    {{ grupo[0].pedido_grupo
                      ? grupo[0].pedido_grupo.slice(0, 20) + '...'
                      : '#' + grupo[0].id }}
                  </span>
                </div>
            ...
            <div class="grupo-footer">
              ...
              <span v-if="grupo[0].fecha_estado_actualizado" class="grupo-fecha-estado">
                🕐 Último cambio: {{ formatFecha(grupo[0].fecha_estado_actualizado) }}
              </span>
              <!-- HU-18: factura disponible desde el primer momento -->
              <template v-if="facturasCliente[grupo[0].pedido_grupo]">
                <button
                  class="btn-ver-factura"
                  @click="verFacturaCliente(facturasCliente[grupo[0].pedido_grupo].id)">
                  🧾 {{ facturasCliente[grupo[0].pedido_grupo].numero_factura }} — Ver PDF
                </button>
                <button
                  class="btn-ver-factura-json"
                  @click="descargarJSONDesdeHistorial(facturasCliente[grupo[0].pedido_grupo].id, facturasCliente[grupo[0].pedido_grupo].numero_factura)">
                  { } JSON
                </button>
              </template>
            </div>
            ...
            <div v-else>
              <div v-for="item in carrito" :key="item.id" class="carrito-item">
                <div class="carrito-img">
                  <img v-if="item.imagen" :src="item.imagen" :alt="item.nombre"
                    class="carrito-foto" @error="item.imagen = null" />
        ...
        <div class="modal-card">
          ...
          <div class="modal-body">
            ...
            <button
              ...
              🛒 Agregar {{ modalCantidad }} al carrito —
              ${{ (modalProducto.precio * modalCantidad).toFixed(2) }}
            </button>
          </div>
        </div>
      </div>
...
// ── HU-11: Carrito persistente con localStorage ───────────────
...
function cantidadEnCarrito(id)    { const it = carrito.value.find(i => i.id === id); return it ? it.cantidad : 0 }
function stockDisponible(id)      { const pr = productos.value.find(p => p.id === id); return pr ? pr.stock : 0 }
function stockAgotadoEnCarrito(p) { return cantidadEnCarrito(p.id) >= p.stock }
...
function agregarDesdeModal() {
  ...
  if (existe) {
    existe.cantidad = Math.min(
      existe.cantidad + modalCantidad.value,
      modalProducto.value.stock
    )
  } else {
...
// ── HU-12 + HU-16 + HU-18: Confirmar pedido con factura auto ──
async function confirmarPedido() {
  ...
  try {
    ...
    // HU-16 + HU-18: guardar resumen con factura generada automáticamente
    ordenConfirmada.value = {
      ...
      factura:    res.data?.factura   || null   // null si no hay config empresa
    }
...
// ── HU-18: Facturas del cliente ───────────────────────────────
async function cargarFacturasCliente() {
  try {
    const grupos     = Object.keys(pedidosAgrupados.value)
    const resultados = await Promise.all(
      grupos.map(g => api.get(`/facturas/pedido/${g}`).catch(() => ({ data: null })))
    )
    const mapa = {}
    grupos.forEach((g, i) => {
      if (resultados[i].data) mapa[g] = resultados[i].data
    })
    facturasCliente.value = mapa
  } catch {
    facturasCliente.value = {}
  }
}

// Ver PDF desde historial de pedidos
function verFacturaCliente(id) {
  window.open(
    `${import.meta.env.VITE_API_URL || "http://localhost:3000"}/api/facturas/${id}/pdf`,
    "_blank"
  )
}

// Descargar JSON desde historial de pedidos
async function descargarJSONDesdeHistorial(id, numero) {
  try {
    const res  = await fetch(
      `${import.meta.env.VITE_API_URL || "http://localhost:3000"}/api/facturas/${id}/json`
    )
...
// Descargar PDF desde pantalla de resumen (busca por grupo)
async function descargarFacturaPDF(grupo) {
  try {
    const res = await api.get(`/facturas/pedido/${grupo}`)
    if (res.data?.id) {
      window.open(
        `${import.meta.env.VITE_API_URL || "http://localhost:3000"}/api/facturas/${res.data.id}/pdf`,
        "_blank"
      )
    }
  } catch {
...
// Descargar JSON desde pantalla de resumen (busca por grupo)
async function descargarFacturaJSON(grupo) {
  try {
    const res = await api.get(`/facturas/pedido/${grupo}`)
    if (res.data?.id) {
      await descargarJSONDesdeHistorial(res.data.id, res.data.numero_factura)
    }
  } catch {
    alert("Error al descargar el JSON")
  }
...
/* HU-18: fila factura en resumen */
.factura-row    { border-top: 1px solid rgba(29,158,117,0.2); padding-top: 10px; margin-top: 4px; }
.factura-numero { font-family: 'Syne', sans-serif; font-size: 16px; color: #1D9E75; }
.resumen-acciones { display: flex; gap: 12px; }
.btn-secondary { flex: 1; background: var(--bg-card-hover); border: 1px solid var(--border); border-radius: 10px; padding: 11px; font-size: 13px; color: var(--text-secondary); cursor: pointer; font-family: 'DM Sans', sans-serif; transition: all 0.15s; }
.btn-secondary:hover { color: var(--text-primary); }
/* HU-18: botones descarga en resumen */
.factura-download-row { display: flex; gap: 10px; margin-top: 12px; }
.btn-dl-pdf  { flex: 1; background: linear-gradient(135deg, #378ADD, #1D9E75); border: none; border-radius: 10px; padding: 11px; font-size: 13px; font-weight: 600; color: #fff; cursor: pointer; font-family: 'Syne', sans-serif; }
.btn-dl-json { flex: 1; background: rgba(124,58,237,0.12); border: 1px solid rgba(124,58,237,0.3); border-radius: 10px; padding: 11px; font-size: 13px; font-weight: 600; color: #7C3AED; cursor: pointer; font-family: 'DM Sans', sans-serif; }
.btn-dl-json:hover { background: rgba(124,58,237,0.22); }
...
.grupo-footer  { margin-top: 12px; padding-top: 10px; border-top: 1px solid var(--row-border); display: flex; gap: 10px; flex-wrap: wrap; align-items: center; }
.grupo-pago         { font-size: 12px; color: var(--text-muted); }
.grupo-fecha-estado { font-size: 11px; color: var(--text-muted); }
/* HU-18: botones en historial */
.btn-ver-factura { background: rgba(55,138,221,0.12); border: 1px solid rgba(55,138,221,0.3); border-radius: 8px; padding: 5px 12px; font-size: 12px; font-weight: 600; color: #378ADD; cursor: pointer; font-family: 'DM Sans', sans-serif; transition: all 0.15s; margin-left: auto; }
.btn-ver-factura:hover { background: rgba(55,138,221,0.22); }
.btn-ver-factura-json { background: rgba(124,58,237,0.1); border: 1px solid rgba(124,58,237,0.25); border-radius: 8px; padding: 5px 10px; font-size: 12px; font-weight: 600; color: #7C3AED; cursor: pointer; font-family: 'DM Sans', sans-serif; transition: all 0.15s; }
.btn-ver-factura-json:hover { background: rgba(124,58,237,0.2); }
...
.carrito-acciones { display: flex; gap: 10px; margin-top: 14px; }
...
.btn-confirmar { flex: 2; background: linear-gradient(135deg, #378ADD, #1D9E75); border: none; border-radius: 10px; padding: 11px; font-size: 13px; font-weight: 600; color: #fff; cursor: pointer; font-family: 'Syne', sans-serif; transition: opacity 0.2s; }
...
.producto-img    { background: var(--bg-card-hover); display: flex; align-items: center; justify-content: center; height: 160px; cursor: pointer; position: relative; overflow: hidden; }
...
.btn-primary   { background: linear-gradient(135deg, #378ADD, #1D9E75); border: none; border-radius: 10px; padding: 10px 24px; font-size: 13px; font-weight: 600; color: #fff; cursor: pointer; font-family: 'Syne', sans-serif; margin-top: 12px; flex: 1; }
...
.modal-img      { background: var(--bg-card-hover); display: flex; align-items: center; justify-content: center; height: 200px; position: relative; overflow: hidden; }
...
@media (max-width: 768px) {
  ...
  .factura-download-row { flex-direction: column; }
  .paso-label       { font-size: 9px; max-width: 60px; }
  .toast-notif      { bottom: 16px; right: 16px; left: 16px; max-width: unset; }
}
</style>