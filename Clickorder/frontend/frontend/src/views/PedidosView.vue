    ...
    <div class="page-content">
      ...
      <div class="resumen-row">
        ...
        <div class="resumen-card">
          ...
          <span class="resumen-num naranja">{{ pedidos.filter(p => p.estado === 'Pendiente').length }}</span>
        </div>
        <div class="resumen-card">
          <span class="resumen-label">Entregados</span>
          <span class="resumen-num verde">{{ pedidos.filter(p => p.estado === 'Entregado').length }}</span>
        </div>
      </div>
        ...
        <table>
          ...
          <tbody>
            <tr v-if="pedidos.length === 0">
              <td colspan="9" class="empty">No hay pedidos registrados</td>
            </tr>
            <tr v-for="p in pedidos" :key="p.id">
              ...
              <td>
                <select
                  ...
                  :class="['estado-select', estadoClase(p.estado)]">
...
.resumen-row { display: flex; gap: 16px; margin-bottom: 24px; flex-wrap: wrap; }
.resumen-card { flex: 1; min-width: 140px; background: var(--bg-card); border: 1px solid var(--border-soft); border-radius: 12px; padding: 16px 20px; display: flex; flex-direction: column; gap: 4px; }
.resumen-label { font-size: 11px; text-transform: uppercase; letter-spacing: 0.06em; color: var(--text-muted); }
.resumen-num { font-family: 'Syne', sans-serif; font-size: 26px; font-weight: 700; color: var(--text-primary); }
...
th { padding: 12px 14px; text-align: left; font-size: 11px; font-weight: 500; letter-spacing: 0.06em; text-transform: uppercase; color: var(--text-muted); }
tbody tr { border-top: 1px solid var(--row-border); transition: background 0.15s; }
tbody tr:hover { background: var(--row-hover); }
td { padding: 12px 14px; font-size: 13px; color: var(--text-secondary); }
...
.cantidad-badge { background: rgba(55,138,221,0.15); border: 1px solid rgba(55,138,221,0.25); border-radius: 6px; padding: 3px 8px; font-size: 12px; color: #378ADD; }
.estado-select { border: none; border-radius: 999px; padding: 4px 10px; font-size: 11px; font-weight: 500; cursor: pointer; font-family: 'DM Sans', sans-serif; outline: none; }
.estado-pendiente  { background: rgba(239,159,39,0.15);  color: #EF9F27; }
.estado-proceso    { background: rgba(55,138,221,0.15);  color: #378ADD; }
.estado-entregado  { background: rgba(29,158,117,0.15);  color: #1D9E75; }
